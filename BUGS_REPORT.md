...
I chose to use a testing platform called Playwright. It is a front-end tesint platform that executes tests in browser. I chose this framework because it is what I have most experience with. I did not perform unit testing because I am not familiar with doing this. In my current position, app devs do these while my team focuses on the front-end/user experience testing. Using Playwright indirectly test most features of apps but it is hard to isolate individual features. I do my best to isolate features by writing individual tests for each feature, rather than writing one test that touches all features. This also helps with identifying bugs after the tests run. Playwright produces a report after the tests have run. The report contains pass, fail and flakey logs, with the options to include videos, screenshots and trace logs of each individual test.
